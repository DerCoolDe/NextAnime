...
const DAY_GROUP_STYLE = { marginBottom: "clamp(20px, 4vw, 35px)" };
...
const LIST_STYLE = { listStyle: "none", padding: 0, margin: 0 };
const LIST_ITEM_STYLE = { marginBottom: "clamp(8px, 2vw, 15px)" };

function UpcomingAnimeVertical({ episodes, watchingIdSet, onAddAnime }) {
  ...
  return (
    <div>
      <h3 style={SECTION_TITLE_STYLE}>Upcoming Episodes</h3>

      {sortedDays.map((dayKey) => (
        <div key={dayKey} style={DAY_GROUP_STYLE}>
          <h4 style={DAY_HEADER_STYLE}>{dayKey}</h4>
          <div style={DAY_BODY_STYLE}>
            <ul style={LIST_STYLE}>
              {groupedEpisodes[dayKey].map((ep) => (
                <li key={ep.media.id + ep.episode} style={LIST_ITEM_STYLE}>
                  <AnimeCard
                    episode={ep}
...
export default React.memo(UpcomingAnimeVertical);
